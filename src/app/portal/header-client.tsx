    ...
    <div ref={rootRef} className="relative">
      ...
      {open ? (
        <div className="absolute right-0 mt-2 w-44 bg-white border rounded-xl shadow-lg p-1 z-30">
          <button
            className="w-full text-left px-3 py-2 rounded hover:bg-gray-50 text-base"
            onClick={() => {
          ...
          {role === "ADMIN" ? (
            <Link href="/admin" className="block px-3 py-2 rounded hover:bg-gray-50 text-xl font-semibold" onClick={() => setOpen(false)}>
              管理后台
            </Link>
          ) : null}

          <button
            className="w-full text-left px-3 py-2 rounded hover:bg-gray-50 text-base text-red-600"
            onClick={() => {
              setOpen(false);
