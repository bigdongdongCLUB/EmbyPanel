...
export function MonitoringAnomaliesClient() {
  ...
  async function refresh() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json?.error ? JSON.stringify(json) : `HTTP ${res.status}`);
      setData(json);
      if (json?.penaltyConfig) {
        setPenaltyEnabled(!!json.penaltyConfig.enabled);
        setPenaltyDuration(Number(json.penaltyConfig.durationMinutes || 5));
      }
    } catch (e: any) {
        ...
        <div className="mt-3 overflow-auto">
          <table className="min-w-[900px] w-full text-sm">
            ...
            <tbody>
              ...
              {data && (data.penaltyRecords?.length ?? 0) === 0 ? (
                <tr>
