...
export type PlaybackStatsPayload = {
  ok: true;
  rangeDays: PlaybackStatsRangeDays;
  summary: {
...
export async function getPlaybackStatsForUsername(params: { username: string; rangeDays: PlaybackStatsRangeDays }): Promise<PlaybackStatsPayload> {
  const username = String(params.username || "").trim();
  const rangeDays = params.rangeDays;
